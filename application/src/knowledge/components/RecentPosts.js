...
export default class RecentPosts extends Component {
  ...
  componentDidMount()
    {
      fetch(this.JIVE_API_URI+'places/1373/contents',{
    method:'GET',
    headers:{
    ...
    componentWillUnmount(){
     ...
     rowPressed(rowData) {
       ...
       this.props.navigator.push({
         route: 'post-view',
         passProps: {auth: this.props.auth, uri: rowData.resources.html.ref, usrName: this.state.usrName, usrAvatar: this.state.usrAvatar, designation: this.state.designation, title: 'Knowledge Network'}
       });
     }
     _renderRow(rowData) {
       if(!this.state.error){
         date = rowData.updated.split('T');
       ...
       if(this.state.loading){
         return(
           <View style={styles.container}>
             <Header navigator={this.props.navigator} title={'Knowledge Network'} />
             <View style={styles.separator}/>

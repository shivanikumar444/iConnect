...
import { StyleSheet, View, Text, ListView, TouchableHighlight, Image, Dimensions,AsyncStorage, Alert } from 'react-native';
...
export default class HRRecentPosts extends Component {
  JIVE_API_URI = ''
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    fetch(this.JIVE_API_URI+'places/2302/contents?sort=dateCreatedDesc', {
             method: 'GET',
             headers: {
           ...
           .then((jsonResponse) => {
             ...
             this.setState({
               dataSource: this.state.dataSource.cloneWithRows(jsonResponse.list),
               loading: false
             });
           })
           .catch((error) => {
      ...
      componentWillUnmount(){
       ...
       rowPressed(rowData) {
         ...
         this.props.navigator.push({
           route: 'post-view',
           passProps: {auth: this.props.auth, uri: rowData.resources.html.ref, usrName: this.state.usrName, usrAvatar: this.state.usrAvatar, designation: this.state.designation, title: 'HR'}
         });
       }
  _renderRow(rowData) {
    if(!this.state.error){
   ...
   if(this.state.loading){
     return(
       <View style={styles.container}>
         <Header navigator={this.props.navigator} title={'HR'} />
         <View style={styles.separator}/>
